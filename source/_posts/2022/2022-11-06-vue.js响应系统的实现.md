...
function track(target, key) {
  if (activeEffect) {
    let effectsGroup = bucket.get(target);
    if (!effectsGroup) {
      bucket.set(target, (effectsGroup = new Map()));
    }
    let effects = effectsGroup.get(key);
    if (!effects) {
      effectsGroup.set(key, (effects = new Set()));
    }
    effects.add(activeEffect);
  }
}
function trigger(target, key) {
  const effectsGroup = bucket.get(target);
  if (effectsGroup) {
    const effects = effectsGroup.get(key);
    effects && effects.forEach((fn) => fn(target));
  }
}
// 首次渲染
useEffect((data) => {
  document.body.innerText = data.text;
}, obj1); // 这里也要传obj了，不然get的时候，也是监听不到的
useEffect((data) => {
  document.body.setAttribute("b", data.b);
}, obj2);
// set操作，触发重新渲染
obj1.text = "hello aaaa"; // 这里必须要修改obj对象（改data是监听不到的）
obj2.b = "333";
```
