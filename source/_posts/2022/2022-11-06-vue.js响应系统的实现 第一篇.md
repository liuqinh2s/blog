...
> 注意：从这里开始写的不对，actvieEffect 并没有记录对，set 的时候会执行响应函数，响应函数会触发 get，get 会重新添加响应函数，这个时候 activeEffect=fn 这一句并没有重新执行（只在初始化的时候执行了），这里要改。
...
目前来说首次执行副作用函数会触发 get，get 会把副作用函数添加为响应函数，然后 set 会执行响应函数，执行响应函数又会触发 get，如此往复循环。

每次都会重新添加响应函数，似乎很冗余，能否只执行一次呢？答案是不能，因为可能存在分支，例如：
...
我们改写了 useEffect，把 activeEffect 包装了一下，增加了一个 records 来记录需要清理哪些集合，在每次重新执行。并在 track 函数里面对这个 records 进行填充
