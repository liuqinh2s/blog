...
function maxSlidingWindow(nums: number[], k: number): number[] {
    const pq = new PriorityQueue1({ compare: (a: { num: number; index: number }, b: { num: number; index: number }) => a.num - b.num });
    let res = [];
    let i = 0;
    let max = nums[i];
    for (let j = i; j < i + k; j++) {
        pq.enqueue({ num: nums[j], index: j })
    }
    res.push(pq.front().num);
    for (i = k; i < nums.length; i++) {
        while (pq.front() && pq.front().index < i-k+1) {
            pq.dequeue();
        }
        pq.enqueue({ num: nums[i], index: i })
        res.push(pq.front().num);
    }
    return res;
};
