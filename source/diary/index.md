...
时间复杂度是 $O(n^2)$，因为里层双指针拼起来是 n。排序的时间复杂度是 $O(n\log n)$。所以总的时间复杂度还是 $O(n^2)$。
...
当然对于那种含着金汤匙出生或者已经完成资本的原始积累，理论上不再需要劳动来换取钱财的人，不在这个体系之列。他们对应着那些已经通关星露谷物语的玩家。

如果是白手起家，高收入起步时有两条路，不过最终都通往资本的道路：
...
-r = --remotes，输出带origin/分支名，就是你rebase origin/main用到的名称
-a = --all：本地 + 远程分支一起列出 git branch -a
-rv：附带每个远程分支最新 commit：git branch -rv
...
另外就是必须要专注于一个大类垂直领域，不要跨大类去发内容，这样一个是粉丝转化率不高，另一个是掉粉严重。
...
- [腾讯云服务器](https://cloud.tencent.com/login)：现在99一年的2核2G的服务器腾讯云和阿里云都有，挺便宜的。
- [LIU冀杨的科技日报](https://jiyanggg.github.io/ai-daily-site/)：我堂弟做的科技日报用AI筛选科技新闻
...
建议在平板和手机上面使用 Wayland，启动的过程需要一点时间，但是启动过后不管是 xfce 本身还是里面的程序运行的速度更快，但是 wayland 也有一些缺点，就是有部分较老的应用程序不支持，例如谷歌拼音需要 fcitx5 以后的版本。
...
一直都想测一下新买的火影 t6a 的跑分，我朋友写了个奇怪的测试 CPU 的程序：[最小生成树 kruskal 算法 js 实现](http://zengyuxin.com/mini-span-tree-kruskal-js/)，测试结果是插电源开 balance 模式，118ms。还行
